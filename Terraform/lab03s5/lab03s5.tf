...
resource "azurerm_network_interface" "lab03s3-nic" {
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.lab03s3-pip.id
  }
  # lifecycle {
...
resource "azurerm_public_ip" "lab03s3-pip" {
...
# OS disk name.
resource "azurerm_linux_virtual_machine" "lab03s3-vm" {
  ...
  network_interface_ids = [
    azurerm_network_interface.lab03s3-nic.id,
  ]
